sap.ui.define([], function () {
   ...
   * HTML5-repo-runtime path (e.g. "/poc.sp.hub.sourcingproject/index.html").
   ...
   * intents (e.g. "#SourcingProject-manage"), so a raw path 404s there.
   *
  ...
  return {
     ...
     *
     * @param {string} sSemanticObject e.g. "SourcingProject"
     * @param {string} sAction e.g. "manage"
     * @param {object} [mParams] navigation parameters, e.g. { id: sProjectId }
     * @param {string} sFallbackPath direct HTML5 path used when no shell container
     *   is present, e.g. "/poc.sp.hub.sourcingproject/index.html#/SourcingProjects(...)"
     * @param {string} [sInnerAppRoute] target app's own hash route (e.g.
     *   Fiori Elements' object-page key), passed through as hrefForExternal's
     *   appSpecificRoute so the target app lands on the right object page
     *   even though the intent has no declared navigation parameter for it.
     ...
     */
    openIntent: function (sSemanticObject, sAction, mParams, sFallbackPath, sInnerAppRoute, oNewTab) {
      if (sap.ushell && sap.ushell.Container) {
        sap.ushell.Container.getServiceAsync("CrossApplicationNavigation").then(function (oService) {
          var sHash = oService.hrefForExternal({
            target: { semanticObject: sSemanticObject, action: sAction },
            params: mParams || {},
            appSpecificRoute: sInnerAppRoute
          });
          // hrefForExternal returns just the shell-relative hash (e.g.
          // "#SourcingProject-manage?..."), meant to be applied to the CURRENT
          // window's location. A fresh tab has no shell state to resolve it
          // against.
          //
